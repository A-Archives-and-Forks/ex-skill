...
| 来源 | 消息记录 | 备注 |
|------|:-------:|------|
| 微信 PC 端（自动采集） | ✅ SQLite | 需 PC 端登录，自动提取密钥解密 |
| iMessage（macOS） | ✅ SQLite | 海外用户适用，直接读取 chat.db |
| 图片 / 截图 | ✅ | 手动上传 |
| 直接粘贴文字 | ✅ | 手动输入 |
...
> 微信自动采集需要 Windows 环境 + PC 端微信登录，详见 [INSTALL.md](INSTALL.md)
...
- 微信自动采集需 Windows + PC 端微信登录状态
- iMessage 需 macOS，直接读取本地 chat.db
